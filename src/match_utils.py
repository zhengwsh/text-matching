...
def comU1(x, y):
    result = [layer_utils.cosine_distance(x, y), layer_utils.euclidean_distance(x, y),
              layer_utils.l1_distance(x, y)]  # , compute_euclidean_distance(x, y), compute_l1_distance(x, y)
    return tf.stack(result, axis=1)


def comU2(x, y):
    result = [layer_utils.cosine_distance(x, y),
              layer_utils.euclidean_distance(x, y)]  # , compute_euclidean_distance(x, y)
    return tf.stack(result, axis=1)
...
def siameseCNN_match_func(question_context_outputs, passage_context_outputs, context_cnn_dim):
    # Sentence matching layer
    with tf.name_scope("match_sentences_context"):
        sentence_difference = question_context_outputs - passage_context_outputs
        sentence_product = question_context_outputs * passage_context_outputs
        ...
        match_representation = tf.concat([question_context_outputs, sentence_product,
                                          sentence_difference, passage_context_outputs], 1)
        match_dim = 4 * context_cnn_dim  # tf.shape(match_representation)[-1]
    return (match_representation, match_dim)


def matchpyramid_match_func(in_question_repres, in_passage_repres):
    with tf.name_scope("match_sentences_context"):
        match_representation = tf.einsum('abd,acd->abc', in_question_repres, in_passage_repres)
    return match_representation


def esim_match_func(question_context_outputs, passage_context_outputs, context_lstm_dim):
    # Sentence matching layer
    with tf.name_scope("match_sentences_context"):
        sentence_difference = question_context_outputs - passage_context_outputs
        sentence_product = question_context_outputs * passage_context_outputs
        # Shape: (batch_size, 4 * 2 * rnn_hidden_size)
        match_representation = tf.concat([question_context_outputs, sentence_product,
                                          sentence_difference, passage_context_outputs], 2)
        match_dim = 4 * 2 * context_lstm_dim  # tf.shape(match_representation)[-1]
    return (match_representation, match_dim)


def my_match_func(question_context_outputs, passage_context_outputs, context_cnn_dim):
    # Sentence matching layer
    with tf.name_scope("match_sentences_context"):
        sentence_difference = question_context_outputs - passage_context_outputs
        sentence_product = question_context_outputs * passage_context_outputs
        sentence_max = tf.maximum(question_context_outputs * question_context_outputs,
                                  passage_context_outputs * passage_context_outputs)
        # Shape: (batch_size, 5 * cnn_size)
        match_representation = tf.concat([question_context_outputs, sentence_product, sentence_max,
                                          sentence_difference, passage_context_outputs], 1)
        match_dim = 5 * context_cnn_dim  # tf.shape(match_representation)[-1]
    return (match_representation, match_dim)


def cal_blue_matching(in_question, in_passage, max_order=4):
    return


def cal_gram_matching(in_question, in_passage, max_gram=4):
    input_shape = tf.shape(in_question)
    batch_size = input_shape[0]
    question_len = input_shape[1]
    input_shape = tf.shape(in_passage)
    passage_len = input_shape[1]

    features = []
    for gram in range(max_gram):
        intersection = tf.shape(tf.sets.set_intersection(in_question, in_passage))[-1]
        union = tf.shape(tf.sets.set_union(in_question, in_passage))[-1]

        features.append(tf.cast(intersection, tf.float64))
        features.append(tf.cast(union, tf.float64))
        features.append(intersection / union)

    features = tf.concat(axis=1, values=features)
    return features


def my_bilateral_match_func(in_question_repres, in_passage_repres,
                         question_lengths, passage_lengths, question_mask, passage_mask, input_dim, is_training,
                         options=None):
    question_aware_representatins = []
    question_aware_dim = 0
    ...
    #### esim attention
    # att_question_contexts, att_passage_contexts = layer_utils.soft_attention_alignment(in_question_repres, in_passage_repres)
    # question_aware_representatins.append(att_question_contexts)
    # question_aware_dim += input_dim
    # passage_aware_representatins.append(att_passage_contexts)
    ...
    qa_aggregation_input = question_aware_representatins
    pa_aggregation_input = passage_aware_representatins
    print(question_aware_representatins.get_shape())
    print(passage_aware_representatins.get_shape())

    aggregation_representation = tf.concat(axis=1, values=[tf.reduce_max(qa_aggregation_input, axis=1),
                                                            tf.reduce_mean(qa_aggregation_input, axis=1),
                                                            tf.reduce_max(pa_aggregation_input, axis=1),
                                                            tf.reduce_mean(pa_aggregation_input, axis=1),
                                                            ])
    aggregation_dim = question_aware_dim * 4
