...
class Vocab(object):
    def __init__(self, vec_path=None, vec_path2=None, dim=100, fileformat='bin', voc=None, word2id=None, word_vecs=None, unk_mapping_path=None):
        self.unk_label = '<unk>'
        self.stoplist = None
        if fileformat == 'bin':
        ...
        elif fileformat == 'txt4':
            self.fromText_format4(vec_path,vec_path2,voc=voc)
        elif fileformat == 'map':
    ...
    def fromText(self, vec_path,voc=None):
        ...
        for line in vec_file:
            if py == 'py3': line = line.strip()#.decode('utf-8')
    ...
    def fromText_format2(self, vec_path, voc=None, pre_word_vecs=None):
        ...
        if pre_word_vecs is not None:
            ...
            self.word_vecs = np.zeros((self.vocab_size+1, self.word_dim), dtype=np.float32) # the last dimension is all zero
            for cur_index in range(self.vocab_size):
                if len(word_vecs[cur_index]) != self.word_dim: continue
                self.word_vecs[cur_index] = word_vecs[cur_index]
    ...
    def fromText_format4(self, vec_path, vec_path2, voc=None):
        # load freq table and build index for each word
        self.word2id = {}
        self.id2word = {}
        
        vec_file = open(vec_path, 'rt')
        vec_file2 = open(vec_path2, 'rt')
        # header = vec_file.readline()
        # self.vocab_size, self.word_dim = map(int, header.split())
        word_vecs = {}
        for line in vec_file:
        ...
        for line in vec_file2:
            ...
            if line[0] == line[1] == ' ':
                ...
                if len(parts) <= 2: continue
                word = parts[0]
            self.word_dim2 = len(parts[1:])
            if (voc is not None) and (word not in voc): continue
            vector = np.array(parts[1:], dtype='float32')
            word_vecs[self.word2id[word]] = np.append(word_vecs[self.word2id[word]], vector)
        vec_file2.close()

        self.word_dim = self.word_dim+self.word_dim2
        self.vocab_size = len(self.word2id)
        self.word_vecs = np.zeros((self.vocab_size+1, self.word_dim), dtype=np.float32) # the last dimension is all zero
        for cur_index in range(self.vocab_size):
            if len(word_vecs[cur_index]) != self.word_dim: continue
    ...
    def to_char_index_sequence(self, sentence):
        # sentence = sentence.strip().lower()
        sentence = sentence.strip()
        seq = []
        for word in re.split('\\s+', sentence):
    ...
    def dump_to_txt2(self, outpath):
        ...
        for word in self.word2id.keys():
            ...
            if py == 'py3': word= word#.encode('utf-8')
            else: word= word.encode('utf-8')
